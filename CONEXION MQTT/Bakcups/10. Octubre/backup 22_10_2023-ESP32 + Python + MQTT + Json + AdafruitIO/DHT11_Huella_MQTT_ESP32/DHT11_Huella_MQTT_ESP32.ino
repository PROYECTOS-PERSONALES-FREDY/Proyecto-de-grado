...
#define wifi_ssid "LUZA.ETB"
#define wifi_password "AliCama2905"
...
void callback(char* topic, byte* payload, unsigned int length) {
 ...
 if (((char)payload[0])=='*'){
  ...
  else if (configuracion==true && submenu != 1 && ((char)payload[0])=='1') {
    submenu = 1;
    Serial.println("\n\nInscripción del sensor de huellas dactilares de Adafruit");

  } else if (configuracion==true && submenu != 2 && ((char)payload[0])=='2') {
    submenu = 2;
    Serial.println("\n\nEliminar huellas del sensor de huellas dactilares de Adafruit");
  } else if (configuracion==true && submenu != 3 && ((char)payload[0])=='3') {
    submenu = 3;
  }
...
void registrar_huella() {
  ...
  client.publish(fingerprint_password_topic, String("Escriba el número de identificación (del 1 al 127) en el que desea guardar este dedo...").c_str(), true);
  //id = readnumber();
  while (id == 0) {  // Realiza la verificacion: Si id es 0 retorna
    recepcion_datos();
  }
